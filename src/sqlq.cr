...
module Queue
  ...
  class CLI
    ...
    def initialize(@argv = ARGV.dup)
      ...
      @db.exec "CREATE TABLE IF NOT EXISTS queue (id INTEGER PRIMARY KEY, entry TEXT NOT NULL, creation_time TEXT NOT NULL)"
    end
    ...
    def create_id_format
      max = @db.query_one "SELECT MAX(id) FROM queue", as: Int64
    ...
    def cmd_add
      if @arguments.empty?
        ...
        @arguments.each do |entry|
          @db.exec "INSERT INTO queue (entry, creation_time) VALUES (?, ?)", entry, creation_time
        end
        if 1 == @arguments.size
          puts "1 entry added"
        else
    ...
    def cmd_list
      ...
      if limit
        sql = "SELECT id, creation_time, entry FROM queue ORDER BY creation_time, id LIMIT #{limit}"
      else
        sql = "SELECT id, creation_time, entry FROM queue ORDER BY creation_time, id"
      end
    ...
    def cmd_take
      if deleted_entry = @db.query_one? "DELETE FROM queue RETURNING entry ORDER BY creation_time, id LIMIT 1", as: String
        puts deleted_entry
      else
    ...
    def cmd_peek
      if next_entry = @db.query_one? "SELECT entry FROM queue ORDER BY creation_time, id LIMIT 1", as: String
        puts next_entry
      else
        STDERR.puts "no entries"
        exit 1
    ...
    def cmd_delete
      ...
      to_delete.each do |delete|
        ...
        sql = if ENV["SQLQ_DEBUG"]?
                "SELECT id, creation_time, entry FROM queue WHERE " + where_clauses.join(" AND ") + " ORDER BY creation_time, id"
              else
                "DELETE FROM queue WHERE " + where_clauses.join(" AND ") + " RETURNING id, creation_time, entry"
              end
